...
#
# This file is part of EHDENUseCase7.3RemTocNoSubgroupsAgeSex
#
...
exportAnalyses <- function(outputFolder, exportFolder) {
  ...
  cmAnalysisListFile <- system.file("settings",
                                    "cmAnalysisList.json",
                                    package = "EHDENUseCase7.3RemTocNoSubgroupsAgeSex")
...
exportExposures <- function(outputFolder, exportFolder) {
  ...
  pathToCsv <- system.file("settings", "TcosOfInterest.csv", package = "EHDENUseCase7.3RemTocNoSubgroupsAgeSex")
  tcosOfInterest <- read.csv(pathToCsv, stringsAsFactors = FALSE)
  pathToCsv <- system.file("Cohorts.csv", package = "EHDENUseCase7.3RemTocNoSubgroupsAgeSex")
  ...
  createExposureRow <- function(exposureId) {
    ...
    cohortFileName <- system.file("cohorts", paste0(exposureId, ".json"), package = "EHDENUseCase7.3RemTocNoSubgroupsAgeSex")
...
exportOutcomes <- function(outputFolder, exportFolder) {
  ...
  createOutcomeRow <- function(outcomeId) {
    ...
    cohortFileName <- system.file("cohorts", paste0(outcomeId, ".json"), package = "EHDENUseCase7.3RemTocNoSubgroupsAgeSex")
    definition <- readChar(cohortFileName, file.info(cohortFileName)$size)
  ...
  pathToCsv <- system.file("settings", "NegativeControls.csv", package = "EHDENUseCase7.3RemTocNoSubgroupsAgeSex")
...
exportMetadata <- function(outputFolder,
  ...
  database <- tibble(databaseId = databaseId,
                     ...
                     studyPackageVersion = utils::packageVersion("EHDENUseCase7.3RemTocNoSubgroupsAgeSex"),
                     isMetaAnalysis = 0)
...
exportDiagnostics <- function(outputFolder,
  ...
  cluster <- ParallelLogger::makeCluster(min(4, maxCores))
  ParallelLogger::clusterRequire(cluster, "EHDENUseCase7.3RemTocNoSubgroupsAgeSex")
