...
#
# This file is part of EHDENUseCase7.2RemTocNoSubgroupsCCISex
#
...
exportAnalyses <- function(outputFolder, exportFolder) {
  ...
  cmAnalysisListFile <- system.file("settings",
                                    "cmAnalysisList.json",
                                    package = "EHDENUseCase7.2RemTocNoSubgroupsCCISex")
...
exportExposures <- function(outputFolder, exportFolder) {
  ...
  pathToCsv <- system.file("settings", "TcosOfInterest.csv", package = "EHDENUseCase7.2RemTocNoSubgroupsCCISex")
  tcosOfInterest <- read.csv(pathToCsv, stringsAsFactors = FALSE)
  pathToCsv <- system.file("Cohorts.csv", package = "EHDENUseCase7.2RemTocNoSubgroupsCCISex")
  ...
  createExposureRow <- function(exposureId) {
    ...
    cohortFileName <- system.file("cohorts", paste0(exposureId, ".json"), package = "EHDENUseCase7.2RemTocNoSubgroupsCCISex")
...
exportOutcomes <- function(outputFolder, exportFolder) {
  ...
  createOutcomeRow <- function(outcomeId) {
    ...
    cohortFileName <- system.file("cohorts", paste0(outcomeId, ".json"), package = "EHDENUseCase7.2RemTocNoSubgroupsCCISex")
    definition <- readChar(cohortFileName, file.info(cohortFileName)$size)
  ...
  pathToCsv <- system.file("settings", "NegativeControls.csv", package = "EHDENUseCase7.2RemTocNoSubgroupsCCISex")
...
exportMetadata <- function(outputFolder,
  ...
  database <- tibble(databaseId = databaseId,
                     ...
                     studyPackageVersion = utils::packageVersion("EHDENUseCase7.2RemTocNoSubgroupsCCISex"),
                     isMetaAnalysis = 0)
...
exportDiagnostics <- function(outputFolder,
  ...
  cluster <- ParallelLogger::makeCluster(min(4, maxCores))
  ParallelLogger::clusterRequire(cluster, "EHDENUseCase7.2RemTocNoSubgroupsCCISex")
