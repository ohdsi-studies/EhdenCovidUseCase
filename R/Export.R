...
#
# This file is part of EHDENUseCase6.2NoPS
#
...
exportExposures <- function(outputFolder, exportFolder) {
  ...
  pathToCsv <- system.file("settings", "TcosOfInterest.csv", package = "EHDENUseCase6.2NoPS")
  tcosOfInterest <- read.csv(pathToCsv, stringsAsFactors = FALSE)
  pathToCsv <- system.file("Cohorts.csv", package = "EHDENUseCase6.2NoPS")
  ...
  createExposureRow <- function(exposureId) {
    ...
    cohortFileName <- system.file("cohorts", paste0(exposureId, ".json"), package = "EHDENUseCase6.2NoPS")
...
exportOutcomes <- function(outputFolder, exportFolder) {
  ...
  createOutcomeRow <- function(outcomeId) {
    ...
    cohortFileName <- system.file("cohorts", paste0(outcomeId, ".json"), package = "EHDENUseCase6.2NoPS")
    definition <- readChar(cohortFileName, file.info(cohortFileName)$size)
  ...
  pathToCsv <- system.file("settings", "NegativeControls.csv", package = "EHDENUseCase6.2NoPS")
...
exportMetadata <- function(outputFolder,
  ...
  database <- tibble(databaseId = databaseId,
                     ...
                     studyPackageVersion = utils::packageVersion("EHDENUseCase6.2NoPS"),
                     isMetaAnalysis = 0)
...
exportDiagnostics <- function(outputFolder,
  ...
  cluster <- ParallelLogger::makeCluster(min(4, maxCores))
  ParallelLogger::clusterRequire(cluster, "EHDENUseCase6.2NoPS")
