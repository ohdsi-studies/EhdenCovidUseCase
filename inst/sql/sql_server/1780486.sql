...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (4203130)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (4203130)
  and c.invalid_reason is null
...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (19025274)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (19025274)
  and c.invalid_reason is null
...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (1510627,958843,43045764,37103776,37111518,40701753,44157647,42687705,3198592,37497352,37295866,1184818,1207952,43557203,36108881,36108952,37497354,37497353,37497355,36109358,1510628,44168096,36189182,36160510,36787582,36787581,36787580,42875510,36787579,36787578,36787577,42875511,36787576,36787575,42875512,42875513,36503766,36504859,36504599,36508742,1510634,44183141,1510632,44176006,44187244,36787574,44187243,36175651,1510638,36175650,44161445,44180263,40743761,40743760,44166708,42875514,36787573,36787572,44169060,36421026,42875515,44185375,994801,36508651,40743762,37592678,994802,2056664,36508730,36506007,42520701,37103890,40701752,42523151,37106183,40701751,44160497,36787589,36787588,36787587,36787586,36787537,36787585,36787584,36787583,36509872,44171917,40743770,44164714,44176005,40743769,44187242,40743768,44161444,40743766,44187757,40743765,44189254,40743764,40743763,36421027,44161443,44170432,42875516,37592675,37592676,40743767,37592677,994803,2056663,40701750,37104009,42523077,42520682,40701749,37106363,40701748,42533663,35622296,35622583,36787591,36787590,36677336,36680862,1123897,1510629,1510631,44183234,1510635,44179594,1510630,42639793,42639791,42639792,36151100)

) I
) C UNION ALL 
SELECT 11 as codeset_id, c.concept_id FROM (select distinct I.concept_id FROM
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (4239130,19025274)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (4239130,19025274)
  and c.invalid_reason is null

) I
LEFT JOIN
(
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (2514577,2721315,2721316,2787819,2787820,2787821,2787822,4161978,4189560,4347665)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (2514577,2721315,2721316,2787819,2787820,2787821,2787822,4161978,4189560,4347665)
  and c.invalid_reason is null
...
WHERE C.drug_exposure_start_date >= DATEFROMPARTS(2020, 1, 1)
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,-30,P.START_DATE) AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= DATEADD(day,0,P.END_DATE) ) cc 
GROUP BY cc.person_id, cc.event_id
HAVING COUNT(cc.event_id) >= 1
-- End Correlated Criteria
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,-30,P.START_DATE) AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= DATEADD(day,0,P.START_DATE) ) cc 
GROUP BY cc.person_id, cc.event_id
HAVING COUNT(cc.event_id) >= 1
-- End Correlated Criteria
